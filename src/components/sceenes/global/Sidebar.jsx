...
export default function Sidebar({ open, mobileOpen, handleDrawerClose, handleDrawerToggle }) {
    ...
    const handleIsHome = async () => {
        if (user.state === 'not_home') {
            await toggleIsHome({
                body: { state: "home" }, entity_id: userEntity
            }).unwrap();
        } else {
            await toggleIsHome({
                body: { state: "not_home" }, entity_id: userEntity
            }).unwrap();
        }
    }
                    ...
                    <Tooltip title={user.state === "home" ? "leave home" : "come home"}>
                        <ListItemAvatar>
                            <IconButton edge="start" onClick={handleIsHome}>
                                ...
                                >
                                    <Avatar alt="user-profile" src="../../assets/olexii.jpg" />
                                </Badge>
                            </IconButton>
                        </ListItemAvatar>
                    </Tooltip>
                    <ListItemText
                        sx={{ "& .MuiListItemText-primary": { textTransform: "capitalize" } }}
