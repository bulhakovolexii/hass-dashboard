...
export default function Player({ entity_id }) {
    ...
    }, isLoading } = useGetStateQuery(entity_id, { pollingInterval: 3000 })
    ...
    const handlePlayPause = () => {
        ControlMusic({ body: { entity_id: entity_id }, action: 'media_play_pause' })
    }

    const handlePrevTrack = () => {
        ControlMusic({ body: { entity_id: entity_id }, action: 'media_previous_track' })
    }

    const handleNextTrack = () => {
        ControlMusic({ body: { entity_id: entity_id }, action: 'media_next_track' })
    }
                ...
                <CardContent sx={{
                    ...
                    >
                        {player.state === "playing" ?
                            <Marquee
                                speed={20}
                                gradient={true}
                                gradientColor={theme.palette.mode === "dark" ? [31, 42, 64] : [242, 240, 240]}
                                gradientWidth={10}>
                    ...
                    >
                        {player.attributes.media_content_type === 'music' ?
                            player.attributes.media_artist : player.attributes.app_name}
                    </Typography>
                </CardContent>
                <Box sx={{
                    display: 'flex',
